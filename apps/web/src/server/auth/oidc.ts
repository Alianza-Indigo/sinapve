...
// Comparacion de contrasena en tiempo constante. Se compara el hash SHA-256 de
// ambos lados para no filtrar longitud; el valor esperado vive en la variable de
// entorno como texto (SINAPVE_ADMIN_PASSWORD), no se persiste hash alguno.
function passwordMatches(provided: string, expected: string | undefined): boolean {
...
export function isAdminCredentialsConfigured() {
  return Boolean(process.env.SINAPVE_ADMIN_EMAIL && process.env.SINAPVE_ADMIN_PASSWORD && process.env.AUTH_SECRET);
}
  ...
  providers.push(
    Credentials({
      ...
      authorize(raw) {
        ...
        if (!password || !passwordMatches(password, process.env.SINAPVE_ADMIN_PASSWORD)) return null;

...
export async function POST(request: Request, { params }: { params: Promise<{ caseId: string }> }) {
  ...
  if (!parsed.success) return Response.json({ error: "invalid_case_close", issues: parsed.error.flatten() }, { status: 400 });

  const { caseId } = await params;
  try {
    const data = await updateCaseState({ caseId, actor, state: "cerrado", ...parsed.data });

...
import { escalateReferral } from "@/server/data/repository";
...
export async function POST(request: Request, { params }: { params: Promise<{ referralId: string }> }) {
  ...
  if (!parsed.success) return Response.json({ error: "invalid_escalation", issues: parsed.error.flatten() }, { status: 400 });

  const { referralId } = await params;
  try {
    const data = await escalateReferral({ referralId, ...parsed.data, actor });
